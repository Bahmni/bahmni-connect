...
describe("FormHierarchyService", function () {
    ...
    beforeEach(module(function ($provide) {
        formService = jasmine.createSpyObj('formService', ['getAllForms', 'getFormDetail']);
        $provide.value('formService', formService);
    }));
    beforeEach(inject(function (_formHierarchyService_, _$q_, _$rootScope_) {
        formHierarchyService = _formHierarchyService_;
        $scope = _$rootScope_;
        $q = _$q_;
        formService.getAllForms.and.returnValue(specUtil.respondWithPromise($q, allForms));
    }));
    ...
    it("should display information when add more obs controls exist", function () {
        ...
        formService.getFormDetail.and.returnValue(specUtil.respondWithPromise($q, formDetails));
        formHierarchyService.build(observations);
        $scope.$apply();
